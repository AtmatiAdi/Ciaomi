...
public class BtController : MonoBehaviour {
    ...
    // Hulajnoga
    public GameObject UScootPanel;
    public Slider UScootSlider;
    bool IsUScooterRunning = false;
    Image UScootImg;
    public Button UScoonClose;
    public Text Pow_Text;
    public Text Rot_Text;
    public Text Spe_Text;
    public Toggle Seq_Toggle;
    public InputField Seq_InputField;
    public InputField Mag_InputField;
    public InputField Dia_InputField;
    public Toggle Dir_Toggle;
    double rot;
    float time = 0.5f; 
    ...
    IEnumerator Start() {
        ...
        UScootImg = UScootPanel.GetComponent<Image>();
        UScoonClose.onClick.AddListener(() => UScoot(false));
    ...
    void Update() {
        string input = "";
        if (IsUScooterRunning)
        {
            input = BtPlugin.Call<string>("RecivedData") + "";
        }
        else
        {
        ...
        { // if left button pressed...
            ...
            if (Physics.Raycast(ray, out hit) && !IsUScooterRunning)
            {   
                ...
                else if (hit.collider.name == "BtConnected")
                {
                    //LineFollower();
                    PairedDevicePrefab = Instantiate(PairedDeviceObject) as GameObject;
                    PairedDevicePrefab.name = "Program_ciaomi";
                    PairedDevicePrefab.transform.SetParent(this.transform.parent.transform);
                    PairedDevicePrefab.transform.localPosition = new Vector3(0f, 5f - (0.8f * 0), -4f);
                    PairedDevicePrefab.transform.localRotation = new Quaternion(0f, 0f, 0f, 0f);
                    PairedDevicePrefab.SetActive(true);
                    PairedDevicePrefab.transform.Find("Name").transform.GetComponent<TextMesh>().text = "CIAOMI";
                    PairedDevicePrefab.transform.Find("MAC").transform.GetComponent<TextMesh>().text = "LINEFOLLOWER";
                    PairedDevicePrefab.transform.Find("Rssi").transform.GetComponent<TextMesh>().text = "PROGRAM";

                    PairedDevicePrefab = Instantiate(PairedDeviceObject) as GameObject;
                    PairedDevicePrefab.name = "Program_uscoot";
                    PairedDevicePrefab.transform.SetParent(this.transform.parent.transform);
                    PairedDevicePrefab.transform.localPosition = new Vector3(0f, 5f - (0.8f * 1), -4f);
                    PairedDevicePrefab.transform.localRotation = new Quaternion(0f, 0f, 0f, 0f);
                    PairedDevicePrefab.SetActive(true);
                    PairedDevicePrefab.transform.Find("Name").transform.GetComponent<TextMesh>().text = "USCOOT";
                    PairedDevicePrefab.transform.Find("MAC").transform.GetComponent<TextMesh>().text = "HULAJNOGA";
                    PairedDevicePrefab.transform.Find("Rssi").transform.GetComponent<TextMesh>().text = "PROGRAM";
                }
                else if (hit.collider.name == "Program_ciaomi")
                {
                    LineFollower();
                }
                else if (hit.collider.name == "Program_uscoot")
                {
                    UScoot(true);
                }
            }
        ...
        if (IsUScooterRunning)
        {
            //string input = BtPlugin.Call<string>("RecivedData") + "";
            byte power = (byte)(UScootSlider.value);
            ...
            {
                time = 0.5f;
                int magnets = int.Parse(Mag_InputField.text);
                double diameter = double.Parse(Dia_InputField.text);
                double speed = (rot * 2.0 * 60.0 * 2.0) / magnets;  // 2 magnesy na 6step, 60 sec w min, co 0.5 sec
                if (magnets > 0)
                {
                    Rot_Text.text = "Obroty: " + speed.ToString() + "RPM";
                }
                if (diameter > 0)
                {
                    Spe_Text.text = "Prędkość: " + ((int)(speed * diameter * (60.0 / 100000) * 3.14)).ToString() + "Km/h";
                }
                rot = 0;
            }
            Pow_Text.text = "Moc: " + (power*100/127).ToString() + "%";
            bool IsTouch = false;
            foreach (Touch touch in Input.touches)
            {
                if (touch.phase != TouchPhase.Ended && touch.phase != TouchPhase.Canceled)
                {
                    if(touch.position.y < 300) IsTouch = true;
                }
            }
            if (IsTouch)
            {
                byte func = SET_MOTOR_1_SPEED_FORWAD;
                if (Dir_Toggle.isOn) ++func;    // Jezeli jest wsteczny

                if (Seq_Toggle.isOn)
                {
                    int seq_number = int.Parse(Seq_InputField.text);
                    func += (byte)(seq_number * 2);
                    
                }
                UScootImg.color = Color.black;
                SendFuncAndValue(func, power);
            }
            else
            {
                UScootImg.color = Color.white;
                SendFuncAndValue((byte)0, (byte)(0));
            }
        }
    }
    ...
    {
        string Message = "" + (char)b;
        BtPlugin.Call("SendToDevice", Message);
    }

    void SendFuncAndValue(byte func, byte val)
    {
        string Message = "" + (char)func + (char)val;
        BtPlugin.Call("SendToDevice", Message);
    }
    ...
    void UScoot(bool active)
    {
        if (active)
        {
            UScootPanel.SetActive(true);
            IsUScooterRunning = true;

            Seq_InputField.text = "1";
            Mag_InputField.text = "30";
            Dia_InputField.text = "24,4";
            Seq_InputField.text = "1";
        }
        else {
            UScootPanel.SetActive(false);
            IsUScooterRunning = false;
        }
    }
